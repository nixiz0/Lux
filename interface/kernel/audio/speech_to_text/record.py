...
def record_audio(language=LANGUAGE, filename=TEMP_AUDIO_PATH, device_index=MIC_INDEX,
    ...
    st.success(f"Écoute.." if language == 'fr' else f"Listen..")
    ...
    if recording_frames:
        st.warning(f"Transcription en cours.." if language == 'fr' else f"Transcription in progress..")
        ...
        st.error(f"Pas d'audio détecté" if language == 'fr' else f"No audio detected.")
        return False