...
def analyze_tool_to_use(user_prompt, tools, tool_embeddings):
    """
    Analyzes the user prompt to determine the most appropriate tool to use based on similarity of embeddings.

    Parameters:
    user_prompt (str): The user's input prompt.
    tools (dict): A dictionary of available tools with their descriptions and functions.
    tool_embeddings (dict): A dictionary containing tool embeddings and their corresponding IDs.

    Returns:
    str: The response generated by the selected tool or the LLM if no tool matches the prompt.
    """
